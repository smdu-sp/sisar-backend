...
export class RelatorioRRService {
  ...
  async countByUnidade(
    ...
    try {
      ...
      if (Object.keys(lista).length === 0) {
        throw new Error(ERROR_MESSAGES.FALHA_DB_UNIDADES
        );
      }
  ...
  async getRelatorio(mes: string, ano: string) {
    try {
      ...
      const analiseGeralSmul: Record<string, number> = await this.countByUnidade(2, 1, unidades, periodFilter);
      const analiseGeralGrap: Record<string, number> = await this.countByUnidade(2, 2, unidades, periodFilter);

      // Dados por tipo e status - Deferidos (status 3)
      const deferidoGeralSmul: Record<string, number> = await this.countByUnidade(3, 1, unidades, periodFilter);
      const deferidoGeralGrap: Record<string, number> = await this.countByUnidade(3, 2, unidades, periodFilter);

      // Dados por tipo e status - Indeferidos (status 4)
      const indeferidosGeralSmul: Record<string, number> = await this.countByUnidade(4, 1, unidades, periodFilter);
      const indeferidosGeralGrap: Record<string, number> = await this.countByUnidade(4, 2, unidades, periodFilter);
